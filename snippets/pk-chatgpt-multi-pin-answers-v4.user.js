// ==UserScript==
// @name         ChatGPT Multi-Pin Réponses v4
// @namespace    http://tampermonkey.net/
// @version      4.0
// @description  Épinglez plusieurs réponses par conversation ChatGPT avec regroupement par conversation, navigation et gestion des sources améliorée.
// @author       cmondary
...
(function() {
    ...
    // Fonction pour extraire les sources de manière plus robuste
    function extractSources(element) {
        const sources = [];
        
        // Rechercher tous les liens dans l'élément
        ...
        links.forEach(link => {
            ...
            // Différentes stratégies pour détecter les sources
            const isSource = 
                text?.toLowerCase().includes('source') ||
                text?.toLowerCase().includes('référence') ||
                text?.toLowerCase().includes('lien') ||
                href.includes('source') ||
                link.getAttribute('data-source') ||
                link.getAttribute('data-citation') ||
                link.closest('[data-source]') ||
                // Détecter les liens vers des sites web externes
                (href.startsWith('http') && !href.includes('chatgpt.com'));
            
            if (isSource && text && href) {
                sources.push({
                    text: text,
                    href: href,
                    title: link.title || link.getAttribute('aria-label') || text
                });
            }
        });
    ...
    function addPinButtons() {
        ...
        answers.forEach((msg, idx) => {
            ...
            btn.onclick = (e) => {
                ...
                const sources = extractSources(contentContainer);
                    ...
                    html += `
                        ...
                        " onmouseover="this.style.boxShadow='0 2px 8px rgba(0,0,0,0.1)'" onmouseout="this.style.boxShadow='none'">
                            ...
                            <div style="
                                ...
                                margin-bottom: 12px;
                            " onclick="scrollToMessage('${convId}', ${i})">
                                    ...
                                    ${p.sources.map(source => `
                                        <a href="${source.href}" target="_blank" rel="noopener noreferrer" 
                                           ...
                                           style="
                                               ...
                                               border: 1px solid var(--token-border-light, #e8e8e8);
                                           "
                                           onmouseover="this.style.background='var(--token-surface-tertiary, #d0d0d0)'; this.style.transform='translateY(-1px)'"
                                           onmouseout="this.style.background='var(--token-main-surface-tertiary, #e8e8e8)'; this.style.transform='translateY(0)'"
                                        >
                                            🔗 ${source.text.length > 30 ? source.text.substring(0, 30) + '...' : source.text}
                                        </a>
                                    `).join('')}
                                </div>
                            ` : ''}
